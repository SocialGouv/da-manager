...
interface Cadre9Props {
  data: DAData;
  projectName: string;
}

export function Cadre9ServeursComposants({ data, projectName }: Cadre9Props) {
  const serveursValides = data.cadre9_ServeursComposants.serveurs.filter(
  ...
  return (
    <Page size="A4" style={s.page} wrap>
      ...
      {serveursValides.length === 0 ? (
        <View style={s.callout}>
          <Text style={s.emptyText}>Aucun serveur défini</Text>
        </View>
      ) : (
        serveursValides.map((serveur, idx) => (
